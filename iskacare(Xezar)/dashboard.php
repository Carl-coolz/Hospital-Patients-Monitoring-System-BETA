...
// Get weekly trend data for line chart with days of the week
$weekly_trend = [];
...
<body>
    ...
    <div class="main-content">
      <div class="header">
        ...
        <div class="search-bar">
          <span>🔍</span>
          <input type="text" placeholder="Search student record..." id="searchInput" onkeyup="searchPatients()" />
      ...
      <div class="activity-list">
        ...
        if (mysqli_num_rows($recent_activities) > 0) {
            while($activity = mysqli_fetch_assoc($recent_activities)) {
                ...
                echo '<img src="pup.png" alt="Student" />';
                echo '</div>';
  ...
  <script>
    ...
    let patientsChart, queueChart, departmentChart;
    let refreshInterval;
      ...
      patientsChart = new Chart(patientsCtx, {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: 'Patient Admissions by Day of Week'
            }
          }
        }
      });
    ...
    function updateCharts() {
      // Update weekly chart
      fetch('chart_api.php?type=weekly')
        .then(response => response.json())
        .then(data => {
          if (data.error) return;
          patientsChart.data.labels = data.labels;
          patientsChart.data.datasets[0].data = data.data;
          patientsChart.update('none');
        })
        .catch(error => console.error('Error updating weekly chart:', error));

      // Update queue chart
      fetch('chart_api.php?type=queue')
        .then(response => response.json())
        .then(data => {
          if (data.error) return;
          queueChart.data.labels = data.labels;
          queueChart.data.datasets[0].data = data.data;
          queueChart.update('none');
        })
        .catch(error => console.error('Error updating queue chart:', error));

      // Update conditions chart
      fetch('chart_api.php?type=conditions')
        .then(response => response.json())
        .then(data => {
          if (data.error) return;
        ...
        .catch(error => console.error('Error updating conditions chart:', error));
    }
