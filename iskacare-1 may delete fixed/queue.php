                            ...
                            <div style="border: 1px solid #ddd; border-radius: 8px; padding: 20px; background: #f9f9f9;">
                                <div style="display: flex; justify-content: between; align-items: center;">
                                    <div style="flex: 1;">
                                        ...
                                        <?php
                                            // Display a readable Time In. If time_in is a valid UNIX timestamp (>= 1_000_000_000 ~ 2001), format it.
                                            // Otherwise fall back to date_admitted if available, or show 'Unknown'. This avoids showing 1970 dates
                                            // for malformed sample data.
                                            $timeInDisplay = 'Unknown';
                                            if (!empty($patient['time_in']) && is_numeric($patient['time_in'])) {
                                                $ti = (int)$patient['time_in'];
                                                if ($ti >= 1000000000) {
                                                    $timeInDisplay = date('M j, Y h:i A', $ti);
                                                } elseif (!empty($patient['date_admitted']) && $patient['date_admitted'] != '0000-00-00') {
                                                    $timeInDisplay = $patient['date_admitted'];
                                                } else {
                                                    // last resort: show raw integer
                                                    $timeInDisplay = $ti;
                                                }
                                            } elseif (!empty($patient['date_admitted']) && $patient['date_admitted'] != '0000-00-00') {
                                                $timeInDisplay = $patient['date_admitted'];
